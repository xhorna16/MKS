...
static const int key_def[5] =  {7, 9, 3, 2, 12 };
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  static uint32_t delay = 0xffffffff;
	  ...
	  if (key != -1) {
		  delay = HAL_GetTick() + 3000; //stanoveni omezeni casu
		  printf("pressed %d\n", key);
		  if (key == key_def[pos]) {
			  pos++;
		  }
	  ...
	  if (pos == 5) {
		  ...
		  pos = 0;
		  delay = UINT32_MAX //alternativne namisto fff...
	  }
